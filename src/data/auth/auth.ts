...
import { createDevSession, isDevAuthEnabled } from '@/utils/dev-auth';
import { z } from 'zod';
...
export const signUpAction = actionClient
  ...
  .action(async ({ parsedInput: { email, password } }) => {
    // Dev режим: автоматическая регистрация без Supabase
    if (isDevAuthEnabled()) {
...
export const signInWithPasswordAction = actionClient
  .schema(signInSchema)
  .action(async ({ parsedInput: { email, password } }) => {
    // Dev режим: автоматический вход без Supabase
    if (isDevAuthEnabled()) {
      await createDevSession();
      return { success: true, message: 'Dev mode: logged in successfully' };
    }
